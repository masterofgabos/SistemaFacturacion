...
<<<<<<< HEAD
        ...
        //Cambiar a Nula
        private void button3_Click_CambiaraNula(object sender, EventArgs e)
        {
            DatosOC datosAnulacion = new DatosOC();
            datosAnulacion.Id_oc = int.Parse(textBox1.Text);

            int resultadoAnulacion = Metodos.CambiaraNula(datosAnulacion);

            if (resultadoAnulacion > 0)
            {
                MessageBox.Show("Estado de la orden cambiado a Anulado", "Operación Exitosa", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("No se pudo cambiar el estado de la orden", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
